...
while true; do
  # Alle 04:00 AM riavvio il server e controllo aggiornamenti
  current_time=$(date +%H:%M)
  if [ "$current_time" = "04:00" ]; then
    clear
  
    # Termina la sessione tmux 'homebridge' se esiste
    ...
    # Riavvio tmux 'homebridge' senza log su file
  ...
  if pgrep -f "proot -b .* hb" > /dev/null; then
    sleep 5
    continue
  else
    clear
    if tmux new-session -d -s homebridge 'proot -b ~/stat:/proc/stat hb'; then
      echo "✅ Homebridge avviato correttamente"
      sleep 60
    else
      echo "❌ Errore durante l'avvio di Homebridge"
      sleep 60
    fi
  fi
done
